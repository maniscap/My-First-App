...
import { IoMdArrowBack, IoIosArrowDown, IoIosArrowUp, IoMdSunny, IoMdMoon, IoMdBookmark, IoMdSend, IoMdClose, IoMdVolumeHigh, IoMdVolumeOff, IoMdChatbubbles, IoMdGrid, IoMdTrash, IoMdMic, IoMdMicOff, IoMdDownload, IoMdShare, IoMdListBox, IoMdBook, IoMdBookmarks } from 'react-icons/io';
import { useSwipeable } from 'react-swipeable';
import { motion, AnimatePresence } from 'framer-motion';
import jsPDF from 'jspdf';

// The path is perfectly set for a file sitting in src/pages/ pulling from src/utils/
// We now import the specialized multi-agent and continuous generation tracks
import { processWithFarmBrain, generateEducationalBook, generateStoryBook, extendStory } from '../../utils/AiBrain';
...
const Page = React.forwardRef(({ title, children, number, theme, fontSize = 16, bookmarkNode }, ref) => {
...
        }}
      >
      <div style={{fontFamily: "'Times New Roman', Times, serif", width: '100%', height: '100%'}}>
         {children}
      </div>
      </div>
    );
...
  };
  ...
  const generatePDFDocument = (book) => {
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    const titleLines = doc.splitTextToSize(book.topic, 170);
  ...
  const handleDownloadPDF = (e, book) => {
    e.stopPropagation();
    const doc = generatePDFDocument(book);
    doc.save(`${book.topic.substring(0, 30)}.pdf`);
  };

  const handleShareBook = async (e, book) => {
    e.stopPropagation();
    const doc = generatePDFDocument(book);
    const pdfBlob = doc.output('blob');
    const file = new File([pdfBlob], `${book.topic.substring(0, 20)}.pdf`, { type: 'application/pdf' });

    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ title: book.topic, text: `Check out this guide on "${book.topic}" generated by FarmCap!`, files: [file] }).catch(err => console.log('Share cancelled', err));
    } else {
      const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`Check out this guide on "${book.topic}"! Generate your own on the FarmCap App.`)}`;
...
                  <div key={book.id} onClick={() => { setBookData(book.bookData); setBookTopic(book.topic); setCoverImage(book.coverImage || null); setIsStoryMode(book.isStoryMode || false); setStoryContext(book.storyContext || ''); setCurrentPage(0); setChatMessages([]); setIsSavedBooksOpen(false); window.history.pushState({ isSubView: true }, ''); }} style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '15px', background: 'rgba(0, 0, 0, 0.3)', borderRadius: '16px', cursor: 'pointer', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', transition: 'background 0.2s', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.15)', textShadow: '0 1px 2px rgba(0,0,0,0.4)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
                      ...
                      <IoMdTrash size={22} color="#ff8a80" onClick={(e) => {
                        e.stopPropagation();
                      ...
                      }} style={{ opacity: 0.8, flexShrink: 0, padding: '2px' }} onMouseOver={e => e.target.style.opacity = 1} onMouseOut={e => e.target.style.opacity = 0.8} />
                    </div>
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button onClick={(e) => handleDownloadPDF(e, book)} style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', color: '#fff', fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', cursor: 'pointer', transition: '0.2s' }}>
                        <IoMdDownload size={16} /> PDF
                      </button>
                      <button onClick={(e) => handleShareBook(e, book)} style={{ flex: 1, padding: '10px', background: '#25D366', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '13px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', cursor: 'pointer', boxShadow: '0 2px 5px rgba(37, 211, 102, 0.4)' }}>
                        <IoMdShare size={16} /> Share
                      </button>
                    </div>
                  </div>
                ))
...
  const pinchRef = useRef({ startDist: 0, startFontSize: 16 });
  ...
  const [showPageBookmarks, setShowPageBookmarks] = useState(false);

  // Keep slider synced with currentPage when we are not scrubbing
  useEffect(() => {
    if (!isSliding) setSliderValue(currentPage);
...
  };
  ...
  const renderPreviewContent = (pageIndex) => {
    if (pageIndex === 0) return <><div style={{fontSize:'12px', fontWeight:'bold', color: isDark ? '#fff':'#000'}}>{bookTopic}</div><div style={{fontSize:'32px', margin:'15px 0'}}>🌱</div><div style={{fontSize:'11px', color:'#888'}}>Cover</div></>;
    if (pageIndex === 1) return <><div style={{fontSize:'12px', fontWeight:'bold', color: isDark ? '#fff':'#000'}}>Table of Contents</div><div style={{fontSize:'32px', margin:'15px 0'}}>📑</div></>;
    if (pageIndex === totalPages - 1) return <><div style={{fontSize:'12px', fontWeight:'bold', color: isDark ? '#fff':'#000'}}>The End</div><div style={{fontSize:'32px', margin:'15px 0'}}>🏁</div></>;
    const page = bookData[pageIndex - 2];
    if (!page) return null;
    return (
      <>
...
      return (
        <PageCover theme={theme}>
          <div style={{ padding: '30px 20px', width: '100%', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', background: 'transparent' }}>
            <h2 style={{ fontSize: `${Math.max(20, fontSize + 4)}px`, color: isDark ? '#fff' : '#111', borderBottom: `2px solid ${isDark ? '#4db6ac' : '#00695c'}`, paddingBottom: '10px', marginBottom: '20px', fontFamily: "'Times New Roman', Times, serif" }}>Table of Contents</h2>
...
                  <span style={{ fontSize: '16px', fontWeight: 'bold', color: isDark ? '#b2dfdb' : '#004d40', fontFamily: "'Times New Roman', Times, serif" }}>{item.title}</span>
                  <span style={{ fontSize: '14px', color: isDark ? '#fff' : '#000', fontFamily: "'Times New Roman', Times, serif" }}>Page {item.pageIndex + 1}</span>
                </div>
              ))}
            </div>
          </div>
...
      );
    }
    const actualIndex = pageIndex - 2;
    const page = bookData[actualIndex];
    if (!page) return <Page number={pageIndex} title="Page not found" theme={theme} fontSize={fontSize}>This page could not be loaded.</Page>;
    
    const isMarked = pageBookmarks.includes(pageIndex);
    const bookmarkNode = showControls ? (
        <button 
            onClick={(e) => {
                e.stopPropagation();
                setPageBookmarks(prev => isMarked ? prev.filter(p => p !== pageIndex) : [...prev, pageIndex]);
            }} 
            style={{
                position: 'absolute', top: '10px', right: '10px', zIndex: 50,
                background: 'transparent', border: 'none', padding: '0', cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                transition: 'all 0.2s', filter: 'drop-shadow(0 2px 3px rgba(0,0,0,0.3))'
            }}
        >
            <IoMdBookmark size={36} color={isMarked ? '#2196F3' : (isDark ? '#fff' : '#000')} />
        </button>
    ) : null;
    return (
      <Page number={pageIndex} title={page.chapter_title} theme={theme} fontSize={fontSize} bookmarkNode={bookmarkNode}>
        {page.page_content}
      </Page>
    );
  };
        ...
        {showControls && (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
              ...
              <button onClick={toggleSpeech} style={iconBtnStyle} title="Read Aloud">{isSpeaking ? <IoMdVolumeHigh size={24} color={isDark ? '#4db6ac' : '#00695c'}/> : <IoMdVolumeOff size={24}/>}</button>
              <button onClick={handleSaveBook} style={iconBtnStyle} title="Save Book">
                <IoMdBookmark size={24} color={isAlreadySaved ? '#2196F3' : undefined} />
              </button>
              <button onClick={() => setShowPageBookmarks(!showPageBookmarks)} style={iconBtnStyle} title="Page Bookmarks">
                <IoMdBookmarks size={24} color={pageBookmarks.length > 0 ? '#2196F3' : undefined} />
              </button>
...
          }}>
            {/* Previous Page (Side Peeking Kindle Effect) */}
            {showControls && currentPage > 0 && (
              <div style={{ position: 'absolute', top: '2%', bottom: '2%', left: '-102%', width: '100%', background: isDark ? '#18181b' : '#fff', opacity: 0.6, transform: 'scale(0.95)', border: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.1)', borderRadius: '20px', overflow: 'hidden', pointerEvents: 'none', zIndex: -1, boxShadow: '0 5px 15px rgba(0,0,0,0.3)' }}>
                {renderPageContent(currentPage - 1)}
              </div>
            )}

            {/* Next Page (Side Peeking Kindle Effect) */}
            {showControls && currentPage < totalPages - 1 && (
              <div style={{ position: 'absolute', top: '2%', bottom: '2%', right: '-102%', width: '100%', background: isDark ? '#18181b' : '#fff', opacity: 0.6, transform: 'scale(0.95)', border: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.1)', borderRadius: '20px', overflow: 'hidden', pointerEvents: 'none', zIndex: -1, boxShadow: '0 5px 15px rgba(0,0,0,0.3)' }}>
                {renderPageContent(currentPage + 1)}
              </div>
            )}

            <div style={{
              width: '100%', height: '100%', background: showControls ? (isDark ? '#18181b' : '#fff') : 'transparent', overflow: 'hidden',
              borderRadius: showControls ? '20px' : '0px',
              border: showControls ? '3px solid #2196F3' : 'none',
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div 
            ...
            style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'transparent', zIndex: 1010, padding: '15px 20px', boxShadow: 'none' }} 
            className="controls-ignore"
          >
            {/* Scrubbing/Page Flip Preview Card */}
            {isSliding && (
              <div style={{
                position: 'absolute', bottom: '100%', left: '50%', transform: 'translateX(-50%)',
                marginBottom: '15px', width: '140px', height: '180px',
                background: isDark ? '#222' : '#fff',
                border: `2px solid ${isDark ? '#4db6ac' : '#00695c'}`,
                borderRadius: '12px', padding: '15px 10px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)',
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                overflow: 'hidden', textAlign: 'center', zIndex: 1020
              }}>
                {renderPreviewContent(sliderValue)}
              </div>
            )}
            
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              <button onClick={() => setShowGridView(!showGridView)} style={{...iconBtnStyle, padding: '5px'}} title="Grid View">
                <IoMdGrid size={24} color={isDark ? '#aaa' : '#555'} />
              </button>

              <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: '8px' }}>
                <div style={{ fontSize: '13px', fontWeight: 'bold', color: isDark ? '#aaa' : '#555', textAlign: 'center' }}>
                   Page {sliderValue + 1} of {Math.max(1, totalPages)}
                </div>
                <input
                  type="range" min="0" max={Math.max(0, totalPages - 1)} value={sliderValue}
                  onChange={(e) => { setIsSliding(true); setSliderValue(parseInt(e.target.value, 10)); }}
                  onMouseUp={(e) => { setIsSliding(false); setCurrentPage(parseInt(e.target.value, 10)); }}
                  onTouchEnd={(e) => { setIsSliding(false); setCurrentPage(parseInt(e.target.value, 10)); }}
                  style={{ width: '100%', accentColor: isDark ? '#4db6ac' : '#00695c', margin: 0 }}
                />
              </div>
...
      />
      ...
      <PageBookmarksModal
        show={showPageBookmarks} 
        onClose={() => setShowPageBookmarks(false)} 
        bookData={bookData} 
        theme={theme} 
        setCurrentPage={setCurrentPage} 
...
const GridViewModal = ({ show, onClose, bookData, bookTopic, coverImage, theme, setCurrentPage, currentPage }) => {
  const activeShadow = `0 0 0 3px #2196F3`;
  return (
  <AnimatePresence>
    {show && (
      <motion.div
...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: theme === 'dark' ? '#fff' : '#000', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}><IoMdArrowBack size={24} /></button>
          <h3 style={{ margin: 0, color: theme === 'dark' ? '#fff' : '#000' }}>Page Layouts</h3>
        </div>
        <div 
          style={{ flex: 1, overflowY: 'auto', padding: '15px', paddingBottom: '40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '15px', alignContent: 'start' }}
...
const PageBookmarksModal = ({ show, onClose, bookData, theme, setCurrentPage, pageBookmarks, currentPage }) => {
  const activeShadow = `0 0 0 3px #2196F3`;
  return (
  <AnimatePresence>
    {show && (
...
        <div style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '15px', background: theme === 'dark' ? 'rgba(20,20,25,0.6)' : 'rgba(255,255,255,0.4)', borderBottom: theme === 'dark' ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(255,255,255,0.4)' }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: theme === 'dark' ? '#fff' : '#000', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}><IoMdArrowBack size={24} /></button>
          <h3 style={{ margin: 0, color: theme === 'dark' ? '#fff' : '#000' }}>Page Bookmarks</h3>
        </div>
        <div 
          style={{ flex: 1, overflowY: 'auto', padding: '15px', paddingBottom: '40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '15px', alignContent: 'start' }}
          onTouchMove={e => e.stopPropagation()}
...
          {pageBookmarks.length === 0 ? (
               <div style={{gridColumn: '1 / -1', padding: '20px', textAlign: 'center', color: '#888'}}>No pages bookmarked yet. Tap the bookmark icon on any page to save it here.</div>
           ) : (
               pageBookmarks.sort((a,b)=>a-b).map(pageNum => {
...
          <div 
            ...
            style={{ flex: 1, overflowY: 'auto', padding: '15px', paddingBottom: '40px' }}
            onTouchMove={e => e.stopPropagation()}
            onWheel={e => e.stopPropagation()}
          >
