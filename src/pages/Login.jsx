...
import { 
  ...
  RecaptchaVerifier,
  onAuthStateChanged // Import Listener
} from 'firebase/auth';
    ...
    <div style={{...containerStyle, backgroundImage: `url('${bgImage}')`}}>
      <div style={glassCardStyle}>
        <div style={{ marginBottom: '20px' }}>
          ...
          <h2 style={titleStyle}>{isSignup ? 'Join Farm Cap' : 'Login'}</h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '20px 0', color: 'rgba(255,255,255,0.6)', fontSize: '12px', fontWeight: 'bold' }}><div style={{flex: 1, height: '1px', background: 'rgba(255,255,255,0.3)'}}></div><span style={{ margin: '0 10px' }}>OR</span><div style={{flex: 1, height: '1px', background: 'rgba(255,255,255,0.3)'}}></div></div>

        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px' }}>
          <button onClick={() => {setAuthMethod('email'); setOtpSent(false)}} style={authMethod === 'email' ? activeTab : inactiveTab}>
            <svg width="20" height="16" viewBox="0 0 48 48" style={{marginRight:'8px', verticalAlign:'middle'}}><path fill="#4caf50" d="M45,16.2l-5,2.75l-5,4.75v13h10V16.2z"></path><path fill="#1e88e5" d="M3,16.2l5,2.75l5,4.75v13H3V16.2z"></path><path fill="#e53935" d="M35,23.7v13h10V16.2l-10-5.5v13z"></path><path fill="#c62828" d="M3,16.2v20.5h10v-13L3,23.7z"></path><path fill="#fbc02d" d="M35,11.2L24,5.2L13,11.2v12.5L24,17.7l11,6V11.2z"></path></svg> Email
          </button>
          <button onClick={() => {setAuthMethod('phone'); setOtpSent(false)}} style={authMethod === 'phone' ? activeTab : inactiveTab}>📱 Phone</button>
        </div>

        {authMethod === 'email' && <form onSubmit={handleEmail} style={formStyle}><input type="email" placeholder="Email" required style={inputStyle} onChange={(e) => setEmail(e.target.value)} /><input type="password" placeholder="Password" required style={inputStyle} onChange={(e) => setPassword(e.target.value)} /><button type="submit" style={mainBtn}>{isSignup ? 'Create Account' : 'Login'}</button><p style={toggleText} onClick={() => setIsSignup(!isSignup)}>{isSignup ? "Login" : "Create Account"}</p></form>}
        {authMethod === 'phone' && <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp} style={formStyle}>{!otpSent ? <><input type="tel" placeholder="Mobile Number" required style={inputStyle} onChange={(e) => setPhone(e.target.value)} /><div id="recaptcha-container"></div><button type="submit" style={mainBtn}>Send OTP</button></> : <><input type="text" placeholder="OTP" required style={{...inputStyle, textAlign: 'center'}} onChange={(e) => setOtp(e.target.value)} /><button type="submit" style={mainBtn}>Verify</button></>}</form>}
...
// STYLES
const containerStyle = { display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', padding: '20px', backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: 'black' };
const glassCardStyle = { width: '100%', maxWidth: '360px', textAlign: 'center', backgroundColor: 'rgba(255, 255, 255, 0.15)', backdropFilter: 'blur(15px)', WebkitBackdropFilter: 'blur(15px)', padding: '30px', borderRadius: '24px', border: '1px solid rgba(255, 255, 255, 0.2)' };
...
const activeTab = { padding: '8px 20px', backgroundColor: 'white', color: 'black', border: 'none', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center' };
