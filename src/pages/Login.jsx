...
import { signInWithPopup, createUserWithEmailAndPassword, signInWithEmailAndPassword, signInWithPhoneNumber, RecaptchaVerifier } from 'firebase/auth';
...
function Login() {
  ...
  const handleGoogle = async () => {
    try { await signInWithPopup(auth, googleProvider); navigate('/dashboard'); } catch (err) { setError(err.message); }
  };
    ...
    <div style={{...containerStyle, backgroundImage: `url('${bgImage}')`}}>
      <div style={glassCardStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '20px 0', color: 'rgba(255,255,255,0.6)', fontSize: '12px', fontWeight: 'bold' }}>
          <div style={{flex: 1, height: '1px', background: 'rgba(255,255,255,0.3)'}}></div><span style={{ margin: '0 10px' }}>OR</span><div style={{flex: 1, height: '1px', background: 'rgba(255,255,255,0.3)'}}></div>
        </div>

        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '20px' }}>
          <button onClick={() => {setAuthMethod('email'); setOtpSent(false)}} style={authMethod === 'email' ? activeTab : inactiveTab}>
            {/* Red Email Icon */}
            <svg width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{marginRight:'8px', verticalAlign:'middle'}}><path d="M18 0H2C0.9 0 0.0100002 0.9 0.0100002 2L0 14C0 15.1 0.9 16 2 16H18C19.1 16 20 15.1 20 14V2C20 0.9 19.1 0 18 0ZM18 4L10 9L2 4V2L10 7L18 2V4Z" fill="#D44638"/></svg> Email
          </button>
...
const titleStyle = { color: 'white', margin: '0', fontWeight: '800', fontSize: '24px' };
const errorBox = { color: '#ffcdd2', fontSize: '13px', background: 'rgba(255,0,0,0.2)', padding: '10px', borderRadius: '8px', marginBottom: '15px' };
const googleBtn = { width: '100%', padding: '12px', backgroundColor: 'white', color: '#333', border: 'none', borderRadius: '50px', cursor: 'pointer', fontSize: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '600' };
const formStyle = { display: 'flex', flexDirection: 'column', gap: '12px' };
const inputStyle = { width: '100%', padding: '12px 15px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.3)', fontSize: '15px', outline: 'none', backgroundColor: 'rgba(255,255,255,0.8)', boxSizing: 'border-box' };
const mainBtn = { padding: '12px', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold' };
const toggleText = { fontSize: '13px', color: 'white', cursor: 'pointer', marginTop: '10px' };
