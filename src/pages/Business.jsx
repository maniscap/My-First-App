...
function Business() {
  ...
  const [newPost, setNewPost] = useState({ 
    crop: '', qty: '', price: '', seller: '', phone: '', location: '', lat: null, lng: null 
  });
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      setNewPost({ crop: '', qty: '', price: '', seller: '', phone: '', location: '', lat: null, lng: null });
      setShowForm(false);
  ...
  return (
    <div style={{...pageStyle, backgroundImage: `url('${bgImage}')`}}>
      <div style={contentContainer}>
            ...
            {showForm && (
              <div style={glassCard}>
                ...
                <form onSubmit={handlePost} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <input type="text" placeholder="Crop Name (e.g. Cotton)" required style={inputStyle} 
                    value={newPost.crop} onChange={(e) => setNewPost({...newPost, crop: e.target.value})} />
                  
                  <div style={{display:'flex', gap:'10px'}}>
                    <input type="text" placeholder="Quantity (e.g. 1 Ton)" required style={inputStyle} 
                  ...
                  <input type="tel" placeholder="Phone Number" required style={inputStyle} 
                    value={newPost.phone} onChange={(e) => setNewPost({...newPost, phone: e.target.value})} />
              ...
              {loading ? <p style={{textAlign:'center', color:'white'}}>⏳ Loading Market...</p> : 
                crops.map((item) => (
                  <div key={item.id} style={glassItem}>
                    ...
                    {/* Call Button uses the phone number now */}
        ...
        {activeTab === 'buy' && <div style={glassCard}><p style={{textAlign:'center', color:'white'}}>🛒 Buyer Marketplace coming soon...</p></div>}
      </div>
    </div>
  );
}
