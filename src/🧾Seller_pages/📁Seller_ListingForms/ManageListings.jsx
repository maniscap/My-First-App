        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#FAFAFA', overflowY: 'auto', WebkitOverflowScrolling: 'touch', width: '100vw', boxSizing: 'border-box', overflowX: 'hidden', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", Roboto, Helvetica, sans-serif' }}>
            ...
            <div style={{ padding: '8px 20px 40px 20px' }}>
                {loading ? (
                    ...
                    <div className="premium-card" style={{ textAlign: 'center', padding: '60px 20px', marginTop: '10px', background: 'linear-gradient(180deg, #FFFFFF 0%, #FAFAFA 100%)' }}>
                        <div style={{ width: '80px', height: '80px', borderRadius: '40px', backgroundColor: '#EFF6FF', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', boxShadow: '0 8px 24px rgba(0,102,204,0.08)' }}>
                            <PackageOpen size={36} color="#0066CC" strokeWidth={1.5} />
                        </div>
                            ...
                            <div key={item.id} className="premium-card" style={{ 
                                ...
                                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                                        <span style={{ fontSize: '11px', fontWeight: '500', color: '#0066CC', backgroundColor: '#EFF6FF', textTransform: 'uppercase', letterSpacing: '0.5px', padding: '4px 8px', borderRadius: '6px' }}>
                                            {item.listingType.replace('_', ' ')}
                                        </span>
                                        {item.status === 'paused' && (
                                            <span style={{ fontSize: '11px', fontWeight: '500', color: '#DC2626', backgroundColor: '#FEF2F2', padding: '4px 8px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px', letterSpacing: '0.5px' }}>
                                                <Power size={10} strokeWidth={2} /> PAUSED
                                            </span>
                                        )}
                                    </div>
                                    <h3 style={{ margin: '0 0 6px 0', fontSize: '17px', fontWeight: '600', color: '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '-0.2px' }}>
                                        {item.itemName}
                                    </h3>
                                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                                        <span style={{ fontSize: '18px', fontWeight: '600', color: '#0066CC' }}>₹{item.price}</span>
                                        <span style={{ color: '#6B7280', fontSize: '13px', fontWeight: '400' }}>/ {item.unit}</span>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flexShrink: 0 }}>
                                    <button 
                                        ...
                                        className="action-btn"
                                        style={{ width: '40px', height: '40px', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.05)', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#4B5563', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="action-btn"
                                        style={{ width: '40px', height: '40px', borderRadius: '10px', border: 'none', backgroundColor: '#FEF2F2', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#DC2626' }}
                                    >
            ...
            {editModalOpen && selectedItemForEdit && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
                    {/* Blurred Backdrop */}
                    <div onClick={() => setEditModalOpen(false)} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)', animation: 'fadeIn 0.25s ease' }}></div>
                    
                    {/* Sheet */}
                    <div style={{ position: 'relative', backgroundColor: '#FFFFFF', width: '100%', borderTopLeftRadius: '24px', borderTopRightRadius: '24px', padding: '24px 24px 48px 24px', animation: 'slideUpSheet 0.35s cubic-bezier(0.2, 1, 0.3, 1)', boxShadow: '0 -20px 60px rgba(0,0,0,0.1)' }}>
                        <style>{`
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                            <h3 style={{ margin: 0, fontSize: '20px', fontWeight: '600', color: '#111827', letterSpacing: '-0.3px' }}>Listing Options</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            >
                                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: '#FAFAFA', border: '1px solid rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#111827', flexShrink: 0 }}>
                                    <Edit2 size={20} strokeWidth={2} />
                                </div>
                                <div>
                                    <h4 style={{ margin: '0 0 4px 0', fontSize: '16px', fontWeight: '600', color: '#111827' }}>Edit Details</h4>
                                    <p style={{ margin: 0, fontSize: '13px', color: '#6B7280', fontWeight: '400' }}>Modify price, photos, or description</p>
                                </div>
                            </button>
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p style={{ margin: 0, fontSize: '13px', color: '#6B7280', fontWeight: '400' }}>
                                        {selectedItemForEdit.status === 'paused' ? 'Make visible to buyers again' : 'Hide temporarily from marketplace'}
                                    </p>
                                </div>
                            </button>
                        </div>
